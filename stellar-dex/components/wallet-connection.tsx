...
export function WalletConnection() {
  ...
  const handleEmailConnect = async () => {
    ...
    setError(null)
    try {
      const response = await axios.post("http://localhost:3001/user/register", { email })

      if (response.data && response.data.user) {
        setUserData(response.data.user)
        setShowEmailDialog(false) // Close email dialog
        setShowBalances(true) // Open balances dialog after successful connection
      } else {
        throw new Error(response.data.message || "Failed to connect wallet. Invalid response format.")
      }
    } catch (err: any) {
    ...
    try {
      if (typeof window !== "undefined" && (window as any).ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          setUserData({
            email: "", // No email for Metamask connection
            ethPublicAddress: ethPublicAddress,
            ethPrivateAddress: "", // Metamask doesn't expose private key
            stellarPublicAddress: "", // Blank for Metamask
            stellarPrivateAddress: "", // Blank for Metamask
          })
          setIsConnected(true)
          setShowBalances(true) // Open balances dialog
        } else {
  ...
  const handleStellarConnect = () => {
    // This is a placeholder for your Stellar connection logic
    setError("Stellar connection not yet implemented.")
    console.log("Stellar Connect clicked")
  }
  ...
  const handleDisconnect = () => {
    ...
    setEmail("") // Clear email on disconnect
  }
          ...
          <Dialog open={showEmailDialog} onOpenChange={setShowEmailDialog}>
            <DialogContent className="bg-slate-900 border-white/10 text-white">
              ...
              <div className="space-y-4">
                <p className="text-gray-400 text-sm">Enter your email to connect or create your account.</p>
            ...
            <DialogTrigger asChild>
              <Button variant="outline" className="border-white/20 text-white hover:bg-white/10 bg-transparent">
                ...
                {userData?.ethPublicAddress
                  ? `${userData.ethPublicAddress.slice(0, 6)}...${userData.ethPublicAddress.slice(-3)}`
                  : "Connected"}
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-slate-900 border-white/10 text-white">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex items-center space-x-2 p-2 bg-white/5 rounded border border-white/10">
                      <code className="text-sm flex-1 font-mono break-all">{userData?.ethPublicAddress || "N/A"}</code>
